async function buildLocals (template, params = {}, reply) {
  ...
  const { merge, pick, get, isEmpty } = this.app.bajo.lib._
  const { theme, iconset, site, user, lang, i18n } = reply.request
  // const ns = concat([name.split(':')[0]], cfg.i18n.defaultNs)
  const routeOpts = get(reply.request, 'routeOptions.config', {})
  const _meta = { theme, iconset, site, user, lang, i18n, template, routeOpts }
