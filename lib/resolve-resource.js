...
function resolveResource (type, item = '', opts = {}, fallbackHandler) {
  ...
  let file
  if (theme) file = filecheck.call(this, { dir, base, exts, check: `${getPluginDataDir(ns)}/${this.name}/${subSubNs ? (subSubNs + '/') : ''}${type}/_${theme.name}` })
  // check override: common
  if (!file) file = filecheck.call(this, { dir, base, exts, check: `${getPluginDataDir(ns)}/${this.name}/${subSubNs ? (subSubNs + '/') : ''}${type}` })
  // check real: theme specific
  if (theme && !file) file = filecheck.call(this, { dir, base, exts, check: `${this.app[ns].dir.pkg}/${this.name}/${subSubNs ? (subSubNs + '/') : ''}${type}/_${theme.name}` })
  // check real: common
  if (!file) file = filecheck.call(this, { dir, base, exts, check: `${this.app[ns].dir.pkg}/${this.name}/${subSubNs ? (subSubNs + '/') : ''}${type}` })
  if (fallbackHandler) file = fallbackHandler.call(this, { file, dir, base, exts, ns, subSubNs, type })
  if (!file) throw this.error(`Can't find ${type}: %s`, item)
