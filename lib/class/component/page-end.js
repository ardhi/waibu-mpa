async function pageEnd (params = {}) {
  ...
  let tc = ''
  if (!params.attr.noToastContainer && this.toastStack && this.toast) {
    const toasts = []
    ...
    for (const html of get(this, 'locals._meta.flash.notify', [])) {
      const attr = { border: 'side:all width:0', text: 'background:info' }
      toasts.push(await this.buildTag({ tag: 'toast', attr, html }))
    }
    tc = await this.buildTag({ tag: 'toastStack', html: toasts.join('\n') })
  }
  params.append = `${tc}</body></html>`
