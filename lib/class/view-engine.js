...
class ViewEngine {
  ...
  async render (tpl, locals = {}, opts = {}) {
    ...
    opts.qs = qs
    const { file } = mpa.resolveTemplate(`${ns}.template${subSubNs ? ('.' + subSubNs) : ''}:${path}`, opts)
  ...
  async write (content, locals = {}, opts = {}) {
    ...
    locals.page.fullTitle = locals.page.title ? `${locals.page.title} - ${req.t(locals.page.appTitle)}` : req.t(locals.page.appTitle)
