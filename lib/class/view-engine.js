...
class ViewEngine {
  ...
  async write (content, locals = {}, opts = {}) {
    ...
    const reqNs = get(req, 'routeOptions.config.ns')
    const frontMatter = parseObject(parsed.frontMatter, { parseValue: true, i18n: req.i18n, ns: reqNs })
    if (opts.prepend) content = `${opts.prepend}\n${content}`
    if (opts.append) content = `${content}\n${opts.append}`
    let layout = frontMatter.layout ?? qs.layout ?? opts.layout
    if (!opts.partial && layout !== false) {
      ...
      for (const item of ['css', 'scripts']) {
        frontMatter[item] = frontMatter[item] ?? []
        if (isString(frontMatter[item])) frontMatter[item] = [frontMatter[item]]
        fm[item] = fm[item] ?? []
        if (isString(fm[item])) fm[item] = [fm[item]]
        frontMatter[item].unshift(...fm[item])
      }
      if (fm.title && !frontMatter.title) frontMatter.title = fm.title
