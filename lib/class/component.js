...
class Component {
  ...
  async buildTag ({ tag, params, reply, el, locals = {} } = {}) {
    ...
    const { isString, camelCase, isEmpty, template: handler, isFunction, merge } = this._
    ...
    let result = await this[method]({ params, reply, el, locals })
    ...
    const merged = merge({}, locals, { attr: result.attr })
    const handlerOpts = { imports: { _app: this.plugin.app } }
    const cacheItem = await getCachedItem({ store: cache, content: result.html, handler, handlerOpts, maxAge: this.cacheMaxAge })
    result.html = cacheItem.item(merged)
  ...
  async include ({ params, reply, locals = {} } = {}) {
