...
class Component {
  ...
  async buildTag (tag, params, reply) {
    ...
    const { isString, camelCase, isEmpty, template, isFunction } = this.plugin.app.bajo.lib._
    ...
    if (isEmpty(result.html)) return await this._renderTag(tag, result, reply)
    ...
    return await this._renderTag(tag, result, reply)
  }

  async _renderTag (tag, params, reply) {
    const { config } = this.plugin.app.waibuMpa
    const { isSet } = this.plugin.app.bajo
    const { isEmpty, forOwn, isArray, isPlainObject } = this.plugin.app.bajo.lib._
    const { arrayToAttr, objectToAttr } = this.plugin.app.waibuMpa
    ...
    return `<${tag}${attrs}>${params.selfClosing ? '' : params.html}</${tag}>`
  }

  async _tAttr (params, reply) {
    const { importModule } = this.plugin.app.bajo
    const { get, map } = this.plugin.app.bajo.lib._
  ...
  async t (params, reply) {
    const { importModule } = this.plugin.app.bajo
    const { get, isArray } = this.plugin.app.bajo.lib._
    const { attrToObject, attrToArray } = this.plugin.app.waibuMpa
  ...
  async include (params, reply) {
    /*
    const { getTplAndTheme } = this.plugin.app.waibuMpa
  ...
  _getIconset (name, reply) {
    const { find } = this.plugin.app.bajo.lib._
    const { iconsets } = this.plugin.app.waibuMpa
  ...
  async icon (params, reply) {
    ...
    params.tag = 'i'
    const defIconset = this._getIconset('default', reply)
