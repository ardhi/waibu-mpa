...
class Component {
  ...
  async _render (params = {}) {
    ...
    params.attrs = objectToAttrs(params.attr)
    if (params.insertCtagAsAttr || config.theme.component.insertCtagAsAttr) params.attrs += ` ctag="${this.namespace}${params.ctag}"`
  ...
  _getIconset (name) {
    ...
    if (!iconset) {
      name = this.plugin.app.waibuMpa.config.iconset.default
  ...
  _buildOptions (params) {
    const { has, omit, find } = this.plugin.app.bajo.lib._
