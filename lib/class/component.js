...
class Component {
  ...
  _buildUrl (params = {}, { excludes = [], prefix = '?' } = {}) {
    const { qs } = this.plugin.app.waibu
    const { get, cloneDeep, forOwn, omit } = this.plugin.app.bajo.lib._
    const qsKey = this.plugin.app.waibu.config.qsKey

    let query = cloneDeep(get(this, 'locals._meta.query', {}))
    forOwn(params, (v, k) => {
      const key = qsKey[k] ?? k
      query[key] = v
    })
    query = omit(query, excludes)
