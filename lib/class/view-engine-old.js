...
class ViewEngine {
  ...
  async render (tpl, locals = {}, opts = {}) {
    ...
    if (this.history[req.id]) {
      if (last(this.history[req.id].file) === file) throw this.plugin.error('tplLoop%s%s', tpl, file)
  ...
  async write (content, locals = {}, opts = {}) {
    ...
    const parsed = await this.md.parse(content, merge({ readFile: false, parseContent: false }, mpa.config.markdown))
    ...
    if (isEmpty(content) && (opts.tpl ?? '').includes('.template')) {
      content = '<!-- include ' + opts.tpl.replace('.template', '.partial') + '  -->'
    }
    ...
    let layout
    if (!opts.partial) {
      locals.page = merge(locals.page, parseObject(parsed.frontMatter, { parseValue: true, lang: req.lang, ns: reqNs }))
      ...
      const layoutParsed = await this.md.parse(layoutContent, merge({ readFile: false, parseContent: false }, mpa.config.markdown))
      layoutContent = ext === '.md' ? this.md.parseContent(layoutParsed.content, mdOpts) : layoutParsed.content
      const fm = parseObject(layoutParsed.frontMatter, { parseValue: true, lang: req.lang, ns: reqNs })
