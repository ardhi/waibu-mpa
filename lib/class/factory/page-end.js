...
class PageEnd extends BaseFactory {
  constructor (options) {
    super(options)
    ...
    if (!this.params.attr.noToastContainer && factory.toastStack && factory.toast) {
      ...
      if (get(locals, 'error')) {
        ...
        if (details.length > 0) {
          ...
          toasts.push(await this.component.buildTag({ tag: 'toast', attr, html: this.component.req.t(locals.error.orgMessage ?? locals.error.message) }))
        }
      }
