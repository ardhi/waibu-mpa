...
async function buildPage ({ text, reply, locals = {}, opts = {} } = {}) {
  ...
  if (!reply.request.theme) return alert.call(this, text, reply.request.t('Warning: no theme provided!'))
  const theme = find(this.themes, { name: get(reply, 'request.theme') })
  ...
  const iconset = find(this.iconsets, { name: get(reply, 'request.iconset') })
