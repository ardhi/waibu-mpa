...
async function apply ({ $, reply, tag, attr, type }) {
  ...
  if (!cached) {
    ...
    for (const item of items) {
      try {
        if (excluded.includes(item)) throw this.error('\'%s\' is excluded from the resource concatenations', item)
        let url = item
        if (!item.startsWith('http')) {
          const u = new URL(reply.request.url, baseUrl)
        ...
        if (!resp.ok) throw this.error('Response error: %s', resp.status)
        const text = await resp.text()
        if (type === 'css' && includes(['url(".', 'url(.', 'url(\'.'], text)) throw this.error('CSS \'%s\' contains relative path', item)
...
async function concatResources (options) {
  ...
  if (this.config.concatResource.css) await apply.call(this, { $, reply, tag: 'link', attr: 'href', type: 'css' })
  if (this.config.concatResource.scripts) await apply.call(this, { $, reply, tag: 'script', attr: 'src', type: 'js' })
}
