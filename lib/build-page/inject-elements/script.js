export function printScript (script) {
  ...
  const { attribsStringify } = this.app.waibuMpa
  const { isString, omit } = this.lib._
  if (isString(script)) script = { src: script }
  return `<script src="${routePath(script.src)}" ${attribsStringify(omit(script, ['src']))}></script>`
}

export async function collectRegular (type, transformer, options = {}) {
  const { runHook } = this.app.bajo
  const { map, get, isString, camelCase, uniq } = this.lib._
  const { arrangeArray } = this.lib.aneka
  const { req } = options
...
export async function collectInline (type, options = {}) {
  const { runHook } = this.app.bajo
  const { isEmpty, get, camelCase, uniq, trim } = this.lib._
  const { $, req } = options
  const items = []
  for (const item of ['theme', 'iconset']) {
    if (this.config[item] && this.config[item].autoInsert[type]) {
