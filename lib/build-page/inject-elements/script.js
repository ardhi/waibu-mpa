...
async function script (options) {
  ...
  for (const item of ['theme', 'iconset']) {
    if (this.config[item] && this.config[item].autoInsert.scripts) {
      ...
      await runHook(`${this.name}.${item}:beforeInjectScripts`, { scripts, items, req })
      if (scripts) items.push(...scripts)
      await runHook(`${this.name}.${item}:afterInjectScripts`, { scripts, items, req })
    }
  }
  const first = []
  const last = []
  ...
  for (const item of ['theme', 'iconset']) {
    if (this.config[item] && this.config[item].autoInsert.scripts) {
      ...
      await runHook(`${this.name}.${item}:beforeInjectInlineScript`, { inlineScript, items, req })
      if (inlineScript) items.push(inlineScript)
      await runHook(`${this.name}.${item}:afterInjectInlineScript`, { inlineScript, items, req })
    }
  }
