...
export async function collectRegular (type, transformer, options = {}) {
  ...
  let fm = get(options, `locals.frontMatter.${type}`, [])
...
export async function collectInline (type, options = {}) {
  const { runHook } = this.app.bajo
  const { get, camelCase, uniq } = this.app.bajo.lib._
  const { req } = options
  const items = []
  for (const item of ['theme', 'iconset']) {
    if (this.config[item] && this.config[item].autoInsert[type]) {
      const rsc = get(options, `${item}.${camelCase(`inline ${type}`)}`)
      const opts = { items, req }
      opts[type] = rsc
      await runHook(`${this.name}.${item}:${camelCase(`before inject inline ${type}`)}`, opts)
      if (rsc) items.push(rsc)
      await runHook(`${this.name}.${item}:${camelCase(`after inject inline ${type}`)}`, opts)
    }
  }
  return uniq(items)
}
...
async function script (options = {}) {
  ...
  const inline = await collectInline.call(this, 'scripts', options)
