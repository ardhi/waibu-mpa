...
function build (mod, fw) {
  ...
  theme.createComponent = async function ($) {
    const cmp = new Component(fw ? fw.plugin : mod.plugin, $, theme)
...
async function collectThemes (ctx) {
  ...
  const { isFunction, isArray, find, isEmpty, cloneDeep } = this.app.bajo.lib._
