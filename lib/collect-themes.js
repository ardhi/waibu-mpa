...
async function build (mod, fw) {
  ...
  const { omit, merge, find } = this.lib._
  const me = this
...
async function collectThemes (ctx) {
  ...
  const { isFunction, isArray, find, without } = this.lib._

  this.themes = []
  if (this.config.theme === false) {
    this.log.warn('supportDisabled%s', this.log.write('Theme'))
    return
  }
  ...
  // no theme? add default, barebone one
  if (this.themes.length === 0) {
    const theme = new Theme(this, 'default')
    theme.css = ['waibuMpa.virtual:/purecss/pure-min.css']
    const me = this
    theme.createComponent = async function ({ $, iconset, req, reply, locals }) {
      const cmp = new Component({ plugin: me, $, theme, iconset, req, reply, locals })
      await cmp.loadBaseFactories()
      return cmp
    }
    this.themes.push(theme)
