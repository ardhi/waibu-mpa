...
async function addMethods ({ file, component, selector, after, prefix = '' } = {}) {
  const { importModule } = this.app.bajo
  const { camelCase, isPlainObject, isFunction } = this.app.bajo.lib._
  const baseName = path.basename(file, path.extname(file))
  let key = camelCase(`${prefix} ${baseName}`)
...
async function loadComponentMethods (cmp) {
  ...
  for (const file of files) {
    await addMethods.call(this, { file, component, selector, after })
  }
  // extender
  await eachPlugins(async function ({ file }) {
    await addMethods.call(me, { file, component, selector, after, prefix: this.alias })
  }, { glob: 'theme/component/*.js', prefix: this.name })
